const cors=(request)=>({'Access-Control-Allow-Origin':request.headers.get('Origin')||'*','Access-Control-Allow-Headers':'Content-Type, Authorization, X-Storage-Key','Access-Control-Allow-Methods':'GET, POST, OPTIONS'});
const json=(data,status=200,request=null)=>new Response(JSON.stringify(data),{status,headers:{'content-type':'application/json','cache-control':'no-store',...(request?cors(request):{})}});
...
async function removeFile(request,env){const d=await request.json(),owner=email(d.ownerEmail),id=String(d.fileId||''); const row=await env.DB.prepare('SELECT storage_key FROM files WHERE id=? AND owner_email=? AND deleted_at IS NULL').bind(id,owner).first(); if(!row)return json({success:false,error:'File not found'},404,request); await env.FILES.delete(row.storage_key); await env.DB.prepare('UPDATE files SET deleted_at=? WHERE id=?').bind(now(),id).run(); return json({success:true,fileId:id},200,request);}
...
export default {async fetch(request,env){try{const url=new URL(request.url);if(request.method==='OPTIONS')return new Response(null,{status:204,headers:cors(request)});if(request.method==='GET'&&url.pathname==='/health')return json({success:true,service:'d1-storage-worker'},200,request);if(request.method==='POST'&&url.pathname==='/upload'){if(!await uploadAuth(request,env))return json({success:false,error:'Unauthorized'},401,request);return upload(request,env);}if(!auth(request,env))return json({success:false,error:'Unauthorized'},401,request);if(request.method==='POST'&&url.pathname==='/download')return download(request,env);if(request.method==='POST'&&url.pathname==='/history')return history(request,env);if(request.method==='POST'&&url.pathname==='/delete')return removeFile(request,env);return json({success:false,error:'Not found'},404,request);}catch(e){console.error(e);return json({success:false,error:'Storage operation failed'},500,request);}},async scheduled(event,env,ctx){ctx.waitUntil(cleanup(env));}};
